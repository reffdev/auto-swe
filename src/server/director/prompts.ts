...
export function buildPlanningPrompt(opts: {
  ...
  const userParts = opts.verificationMode
    ...
    : [
        ...
        `Verification criteria: ${opts.milestoneVerification}`,
        "",
        "Generate the next 1-5 tasks to make progress toward this milestone. Focus on what's",
        "missing or incomplete. **You are DONE when you've emitted a `next_tasks` block.**",
        "Don't write memory files, don't journal — emit the block and exit.",
      ];
