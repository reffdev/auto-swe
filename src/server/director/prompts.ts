...
export function buildPlanningPrompt(opts: {
  ...
  const systemParts = [
    ...
    "## Goal",
    "",
    "Decide what should happen next on the active milestone, and either call the `createTasks`",
    "tool to commit a batch of work OR (in verification mode) call `advanceMilestone` to commit",
    "the milestone as done. Open-ended exploration without a decision is failure.",
    "",
    "**The canonical way to commit a plan is the `createTasks` tool.** When you're ready, call",
    "it with the batch of 1-5 tasks. The stream ends as soon as it returns successfully —",
    "the scheduler will dispatch what you committed. Do NOT keep investigating after.",
    "",
    "## Two hard rules",
    "",
  ...
  systemParts.push(
    "",
    "## How to commit your plan — call the `createTasks` tool",
    "",
    "When you're ready to commit a batch, call `createTasks` with an array of 1-5 task",
    "objects. Each task object has:",
    "",
    "  - `title` (string, unique within the batch)",
    "  - `description` (string — the executing agent has no other context beyond this and",
    "    the project files; be specific about what to do and how to verify)",
    "  - `type` (string — `code`, `art`, `music`, `sfx`, `review`, `content`, `claude`)",
    "  - `priority` (1=highest, 5=lowest, default 3)",
    "  - `target_files` (string[] — relative paths the task creates or modifies)",
    "  - `acceptance_criteria` (string[] — concrete pass/fail conditions)",
    "  - `depends_on` (number[] — 1-based positions of OTHER tasks IN THIS SAME BATCH that",
    "    must complete first; leave empty when tasks can run in parallel)",
    "  - `needs_human_review` (boolean — true if a human should review before acceptance)",
    "",
    "After `createTasks` returns successfully, your work is DONE — the planner stream ends",
    "and the scheduler dispatches the tasks. Do NOT continue investigating or writing memory.",
    "",
    "## Fallback markdown format (only if you cannot call createTasks)",
    "",
    "If for some reason you cannot use the tool, you may emit a `next_tasks` markdown block",
    "with the same shape, and the parser will pick it up. The tool is preferred — it gives",
  ...
  const instructionsBody = opts.verificationMode
    ? [
        ...
        "   - **`ready: false`** → call `createTasks` with corrective tasks for the specific",
        "     blockers it returned. Do NOT call `advanceMilestone`. After `createTasks`",
        "     returns, your work is DONE.",
        "",
        "## You are DONE when ONE of these has happened",
        "",
        "- You called `advanceMilestone` and it returned success, OR",
        "- You called `createTasks` with at least one corrective task and it returned `committed: true`",
        "",
    ...
    : [
        ...
        "## You are DONE when ONE of these has happened",
        "",
        "- You called `createTasks` with at least one task and it returned `committed: true`, OR",
