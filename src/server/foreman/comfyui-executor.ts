...
export async function executeComfyUITask(
  ...
  try {
    ...
    if (config.mode === "img2img" && config.enhance) {
      ...
      try {
        templateContent = await fsReadFile(templatePath, "utf-8");
      } catch {
    ...
    if (galleryDir) await fsMkdir(galleryDir, { recursive: true });
    ...
    for (let vi = 0; vi < variationCount; vi++) {
      ...
      try {
        ...
        if (galleryDir && result.outputFiles.length > 0) {
          ...
          await fsCopyFile(result.outputFiles[0].localPath, dest);
    ...
    if (isStyleExploration) {
      ...
      await fsCopyFile(allOutputFiles[0].localPath, targetPath);
    ...
    // Clean up temp dir
    try { await fsRm(outputDir, { recursive: true, force: true }); } catch { /* best effort */ }

    // Verify files
    for (const p of savedPaths) {
      if (!(await pathExists(p))) throw new Error(`Failed to copy output file to ${p}`);
    }

    // Git add and commit — skip for style exploration (.swe/ is gitignored)
    if (!isStyleExploration && await pathExists(resolve(project.workdir, ".git"))) {
      const relPaths = savedPaths.map(p => p.replace(project.workdir + "/", "").replace(project.workdir + "\\", ""));
      await addFiles(project.workdir, relPaths);
      const hash = await commitSync(project.workdir, `[Foreman] ${task.title} - Generated by ComfyUI`);
      if (hash) {
...
async function uploadEnhanceSource(
  ...
  const blob = new Blob([await fsReadFile(sourceImagePath)], { type: "image/png" });
...
async function applyStyleLock(
  ...
  const styleLock = await getStyleLock(project.workdir);
  const refPath = await getStyleReferencePath(project.workdir);
  if (!styleLock || !refPath) return workflow;

  // Upload reference image
  const refFilename = styleRefFilename(task.id);
  const formData = new FormData();
  const blob = new Blob([await fsReadFile(refPath)], { type: "image/png" });
