    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        <div className="max-w-lg space-y-5">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider block mb-1.5">Git Remote URL</label>
            <Input value={gitRemote} onChange={(e) => setGitRemote(e.target.value)} placeholder="https://github.com/user/repo.git" />
            <p className="text-xs text-muted-foreground mt-1">Remote repository URL for cloning</p>
          </div>

          <div>
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider block mb-1.5">Git Server Token</label>
            <Input value={gitToken} onChange={(e) => setGitToken(e.target.value)} placeholder="ghp_xxxxxxxxxxxx" type="password" />
            <p className="text-xs text-muted-foreground mt-1">Token for PR creation (stored locally)</p>
          </div>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider block mb-1.5">Model ID (optional)</label>
            <Input value={modelId} onChange={(e) => setModelId(e.target.value)} placeholder="e.g. qwen2.5-coder-32b" />
            <p className="text-xs text-muted-foreground mt-1">Override default model for this project</p>
          </div>
